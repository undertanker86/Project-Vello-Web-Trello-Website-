...
/**
 * Note: Modal is a low-component that MUI uses inside things like Dialog, Drawer, Menu, Popover. Of course we can use Dialog here without any problem
 */
function ActiveCard() {
  ...
  // The function to handle the API update card: title, cover, description, comment....
  const handleApiUpdateCard = async (updateData) => {
    const updatedCard = await updateCardDetailsAPI(activeCard._id, updateData)
    // 01: Update the active card in current modal
    dispatch(updateCurrentActiveCard(updatedCard))
    // 02: Update the active card in activeBoard (nested data)
  ...
  const onUploadCardCover = (event) => {
    ...
    // Call API...
    toast.promise(
      handleApiUpdateCard(reqData).finally(() => event.target.value = ''),
      { pending: 'Updating...' }
    )
  }
    ...
    <Modal
      ...
      <Box sx={{
        ...
        <Box sx={{ mb: 1, mt: -3, pr: 2.5, display: 'flex', alignItems: 'center', gap: 1 }}>
          ...
          {/* Feature 01: Card Title Handling */}
          <ToggleFocusInput
            inputFontSize='22px'
        ...
        <Grid container spacing={2} sx={{ mb: 3 }}>
          ...
          <Grid xs={12} sm={9}>
            ...
            <Box sx={{ mb: 3 }}>
              ...
              {/* Feature 03: Card Description Handling */}
